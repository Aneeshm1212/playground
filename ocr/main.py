...
PDF_PATH = "./Result_Extraction/1T00718C.pdf"
DPI = 200
...
def save_images(pil_images):
    ...
    for image in pil_images:
        image.save("./Result_Extraction/Page_Image/page_" + str(index) + ".png",quality=150)
        index += 1

def line_detect(x):
    for number in range(1,x+1):
        img = cv2.imread("./Result_Extraction/Page_Image/page_"+str(number)+".png", cv2.IMREAD_COLOR)
...
def croping(output,i):
    im = Image.open(r"./Result_Extraction/Page_Image/page_"+str(i)+".png")
    ...
    for j in range(len(output)-1):
        ...
        global count
        im1 = im1.save("./Result_Extraction/Crop_Image/crop_"+str(count)+".png",quality=150)
        count=count+1;

def head_croping(output,i):
    im = Image.open(r"./Result_Extraction/Page_Image/page_1.png")
    for j in range(len(output)-1):
        left = output[j][0]
        right = output[j][2]
        top=output[j][1]
        ...
        im1 = im.crop((left, top, right, bottom))
        global count1
        im1 = im1.save("./Result_Extraction/Header_crop/crop"+str(count1)+".png",quality=150)
        count1=count1+1;
...
if __name__ == "__main__":
    ...
    DIR = './Result_Extraction/Page_Image'
